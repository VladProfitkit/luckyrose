...
?><div class="stores gopro_20" data-firstElement="<?=$arParams['FIRST_ELEMENT_ID']?>"><?
	?><div><?
		...
		if (is_array($arResult['JS']['SKU']) && count($arResult['JS']['SKU']) > 1) {
			?><a class="genamount<?php if (count($arResult['STORES']) < 1 || $arParams['SHOW_GENERAL_STORE_INFORMATION'] == 'Y'):?> cantopen<?endif;?>" data-src="#popupstores_<?=$arParams['~ELEMENT_ID']?>" href="#popupstores_<?=$arParams['~ELEMENT_ID']?>" title="<?=$arParams['MAIN_TITLE']?>"><?
				if ($arParams['GOPRO_USE_MIN_AMOUNT'] == 'Y') {
