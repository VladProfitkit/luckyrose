	...
	<form action="<?=$arResult['ACTION_URL']?>" method="POST">
		...
		<?php foreach ($arResult['SHOW_FIELDS'] as $arField): ?>
			<div class="line clearfix">
				<input<?if(strpos($arField['CODE'], 'PHONE') > 0):?> class="maskPhone"<?endif;?> type="text" name="<?=$arField['CODE']?>" value="<?=$arField['HTML_VALUE']?>" placeholder="<?=$arField['NAME']?><?if($arField['REQUIRED_FIELDS'] == 'Y'):?>*<?endif;?>" />
			</div>
		<?php endforeach; ?>
