...
$(window).on("ready", function(event){
	...
	var displayWindow = function(jData){
		if($(window).width() > 700){
			...
			var $price = $appBasket.find(".price").html(jData["PRICE"]).data({"price": jData["PRICE"].replace("$",""), "discount": jData["OLD_PRICE"]});

			var $allSum = $appBasket.find(".allSum").html('$'+(jData["PRICE"].replace("$","")*100*jData["QUANTITY"])/100);
	...
	var appBasketCalc = function($qty, productID){
		...
		var tmpPriceSum = ((parseFloat($price.data("price"))*10*10) * parseFloat($qty.val()))/100;
...
var formatPrice = function(data) {
	...
	return str + (price[1] != undefined ? "." + price[1].substr(0,2) : "");
}
