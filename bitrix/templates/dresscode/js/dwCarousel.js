    ...
    jQuery.fn.dwCarousel = function(options) {
        ...
        var moveUL = function(event) {
            var direction = event.data.direction == "left" ? "left" : "right",
                ...
                parentItem = item.closest(".cItem");

            if (direction == "left") {
                if (!$main.curPos) {
                    animateValue = "-" + (100 / options.countElement * maxPos) + "%";
                    $main.curPos = maxPos;
                    $(".cItem:last-child").find(".caption a").trigger('click');
                } else {
                    animateValue = "-" + (100 / options.countElement * --$main.curPos) + "%";
                    parentItem.prev().find(".caption a").trigger('click');
                }
            } else {
                if ($main.curPos == maxPos) {
                    $main.curPos = maxPos;
                    $main.curPos = animateValue = 0;
                    $(".cItem:first-child").find(".caption a").trigger('click');

                } else {
                    animateValue = "-" + (100 / options.countElement * ++$main.curPos) + "%";
                    parentItem.next().find(".caption a").trigger('click');
                }
            }
