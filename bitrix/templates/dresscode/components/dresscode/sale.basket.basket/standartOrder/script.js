...
$(function(){
	$(".qty").on("keyup", function(e){
		...
		if(value){
			$this.val(value);
			sumCalc = Number(value * Number($sum.data("price") *10*10))/100;
			$sum.html(formatPrice(sumCalc) + sumStr);
			clearTimeout(flushTimeout);
			flushTimeout = setTimeout(function(){
...
var refreshBuyButton = function () {
	var $sum = $main.find(".sum");
	var orderSum     = 0;

	$sum.each(function(index){
		orderSum += parseFloat($(this).text().replace(/\s+/g, '').replace('$',''));
	});

	if(orderSum > 30){
		$('#newOrderMinimalPrice').hide();
		$('#newOrder').show();
	}else{
		$('#newOrderMinimalPrice').show();
		$('#newOrder').hide();
	}
}
...
var flushParams = function(){
	...
	$allSum.html(allStr + formatPrice(orderSum) );
