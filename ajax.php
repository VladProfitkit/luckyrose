					...
					if(!empty($arElement)){
						...
						if(empty($result)){
							$result = array(
								...
								"message" => "In the near future will contact our Manager to clarify the prices.",
								"success" => true
							);
						}
...
<?$APPLICATION->IncludeComponent(
	...
	false
			...
			if ($USER->IsAuthorized()){
				...
				$result = array(
					"error" => "To vote you need to login",
					"result" => false,
					"heading" => "Error"
				);
			}
				   ...
				   !empty($_GET["PRODUCT_ID"])
					...
					while($ob = $res->GetNextElement()){
						...
						if($arFields["PROPERTY_USER_ID_VALUE"] == $arUsers[0]){
							$result = array(
								"heading" => "Error",
								"message" => "You left a review for this product."
							);
							break;
						}
					...
					if(empty($result)){
						...
						if($PRODUCT_ID = $newElement->Add($arLoadProductArray)){
							$result = array(
								"heading" => "Comment add",
								"message" => "Your comment will be published after moderation.",
								"reload" => true
							);
				...
				if(!empty($_GET["phone"]) && !empty($_GET["id"])){
					...
					if(CModule::IncludeModule("iblock") && CModule::IncludeModule("sale")){
						...
						if(!empty($arElement)){
							...
							if ($arPersonItem = $getPersonType->Fetch()){
								...
								if ($ptype = $db_ptype->Fetch()){
									...
									if ($ar_dtype = $db_dtype->Fetch()){
										...
										$result = array(
											"heading" => "Error",
											"message" => "Error, delivery is not created!",
											"success" => false
										);
									}
									...
									$result = array(
										"heading" => "Error",
										"message" => "Error, the payment system was not created!",
										"success" => false
									);
								}
							...
							if(empty($result)){
								$result = array(
									"heading" => "Your order was successfully sent",
									"message" => "In the near future You will contact our Manager to clarify the order details.",
									"success" => true
								);
							}
						}else{
